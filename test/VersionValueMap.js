...
import { VersionedValueMap, VersionedValue } from '../src/index.js'
...
describe('VersionedValueMap', () => {
      ...
      const x: VersionedValueMap<{ foo: string, bar: number }> = new VersionedValueMap({
        items: {
          ...
          bar: { name: 'bar', records: [
            { value: 1, at: isoStr(30) },
            { value: 2, at: isoStr(20) },
            { value: 3, at: isoStr(10) },
          ] },
        }
      })
      assert(x.get('bar') === 3)
  ...
  describe('addItem()', () => {
    it('can create UpdateOperation to add item', () => {
      ...
      assert.deepEqual(y, {
        $set: { 'items.foo': new VersionedValue({ name: 'foo' }) }
      })
    })

    it('throws an error when the item with the given name already exists', () => {
      const x = new VersionedValueMap({ items: { foo: new VersionedValue({ name: 'foo' }) } })
      assert.throws(() => {
        x.addItem('foo')
      }, /already exists/)
    })
  ...
  describe('add()', () => {
    it('can create UpdateOperation with $set operator when the name is new', () => {
      ...
      assert.deepEqual(op, {
        $set: { 'items.foo': new VersionedValue({ name: 'foo', records: [{ value: 1, at }] }) }
      })
    })
    ...
    it('throws an error when the given name contains "["', () => {
      const x = new VersionedValueMap()
      assert.throws(() => {
        x.add('foo[1', { a: 4, b: 'baz' }, isoStr(11))
      }, /Invalid name format/)
    })
    ...
    it('throws an error when the given name contains "]"', () => {
      const x = new VersionedValueMap()
      assert.throws(() => {
        x.add('foo1]', { a: 4, b: 'baz' }, isoStr(11))
      }, /Invalid name format/)
    })
    ...
    it('can create UpdateOperation with regularized $push operator when the item and its records already exist', () => {
      const x = new VersionedValueMap({
        items: {
          foo: { name: 'foo' },
          bar: { name: 'bar', records: [{ value: 1, at: isoStr(11) }] },
        }
      })
      const at = isoStr(10)
      const op = x.add('bar', 3, at)
      assert.deepEqual(op, {
        $push: { 'items.bar.records': { $each: [{ value: 3, at }] } }
      })
    })
  })
  ...
  describe('$add()', () => {
    it('can add value when it has no item with the given name', () => {
      ...
      assert.deepEqual(y, {
        items: {
          foo: { name: 'foo', records: [{ value: 1, at }] }
        }
      })
    })
    ...
    it('throws an error when the given name contains "."', () => {
      const x = new VersionedValueMap()
      assert.throws(() => {
        x.$add('foo.bar', { a: 4, b: 'baz' }, isoStr(11))
      }, /Invalid name format/)
    })
  ...
  describe('remove()', () => {
    ...
    it('can create regularized UpdateOperation with the given time', () => {
      ...
      const foo = { name: 'foo', records: [
        { value: { a: 3, b: '' }, at: isoStr(30) },
        { value: { a: 1, b: 'bar' }, at: time },
        { value: { a: 2, b: 'baz' }, at: isoStr(4) },
      ]}
      const x = new VersionedValueMap({ items: { foo } })
      const op = x.remove('foo', time)
      assert.deepEqual(op, {
        $pull: { 'items.foo.records': { $eq: { value: { a: 1, b: 'bar' }, at: time } } }
      })
    })
  })
      ...
      assert.deepEqual(y, {
        items: {
          foo: {
            name: 'foo',
            records: [
              { value: { a: 3, b: '' }, at: foo.records[0].at },
              { value: { a: 2, b: 'baz' }, at: foo.records[2].at },
            ]
          }
        }
      })
  ...
  describe('removeNewest()', () => {
    ...
    it('can create regularized UpdateOperation', () => {
      ...
      assert.deepEqual(op, {
        $pull: { 'items.foo.records': { $eq: { value: { a: 2, b: 'baz' }, at: foo.records[2].at } } }
      })
    })
  })
      ...
      assert.deepEqual(y, {
        items: {
          foo: {
            ...
            records: [
              { value: { a: 3, b: '' }, at: foo.records[0].at },
              { value: { a: 1, b: 'bar' }, at: foo.records[1].at },
            ]
          }
        }
      })
